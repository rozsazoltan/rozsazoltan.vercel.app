...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      description: page.description,
      type: "article",
      url: `/docs/tsunamaji/plugins/${guide.slug}`,
      images: [{ url: `/api/og?path=/docs/tsunamaji/plugins/${guide.slug}` }],
    },
    twitter: {
...
export default async function Page({ params }: Props) {
  ...
  if (tabs && !selectedTab) {
    return redirect(`/docs/tsunamaji/plugins/${slug}/${tabs[0].slug}`);
  }
          ...
          <div className="mt-6" data-content="true">
            {tabs ? (
              <TabBar
                tabs={tabs.map((tab) => ({
                  title: tab.title,
                  url: `/docs/tsunamaji/plugins/${slug}/${tab.slug}`,
                }))}
              />
            ) : null}
            <Steps steps={steps} />
...
export async function generateStaticParams() {
  ...
  return guides.filter(({ tile }) => tile.external === undefined).flatMap(({ slug, tabs = [] }) => [
    // examples:
    // - /docs/tsunamaji/plugins/nextjs
    // - /docs/tsunamaji/plugins/vite
    { slug: [slug] },

    // examples:
    // - /docs/tsunamaji/plugins/vite/react
    // - /docs/tsunamaji/plugins/vite/vue
    // - /docs/tsunamaji/plugins/vite/svelte
    ...tabs.map((tab) => ({
      slug: [slug, tab.slug],
    })),
  ]);
